...
import { g as getImage, $ as $$Image, a as $$Layout, b as $$Prose } from './briana-weapons_6vpbGIWf.mjs';
import 'kleur/colors';
import 'html-escaper';
import 'clsx';
...
const $$ImageCaption = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { mobileImgUrl, desktopImgUrl, alt, caption } = Astro2.props;
  ...
  return renderTemplate`${maybeRenderHead()}<figure> <picture> <source media="(max-width: 799px)"${addAttribute(mobileImg.src, "srcset")}> <source media="(min-width: 800px)"${addAttribute(desktopImg.src, "srcset")}> <img${addAttribute(desktopImg.src, "src")}${addAttribute(alt, "alt")}> </picture> <figcaption>${caption}</figcaption> </figure>`;
